...
pub fn verify_proof(
    ...
    // In Groth16, public inputs are in the base field Fq, but we need to convert
    // them to the scalar field Fr for use in group element multiplication
    fn fq_to_fr(fq: &Fq) -> <Bn254 as Pairing>::ScalarField {
        // Extract the u64 value from Fq (for small test values)
        // This assumes the Fq value is small enough to fit in a u64
        let bigint = fq.into_bigint();
        let bytes = bigint.to_bytes_be();

        // Convert to u64, taking the least significant bytes
        let mut arr = [0u8; 8];
        let copy_len = std::cmp::min(8, bytes.len());
        arr[8 - copy_len..].copy_from_slice(&bytes[bytes.len() - copy_len..]);
        let val = u64::from_le_bytes(arr);

        <Bn254 as Pairing>::ScalarField::from(val)
    }
    ...
    // The IC vector structure: IC[0] is for the constant 1, IC[1..] are for public inputs
    //
    // Different handling based on IC length:
    // - If IC.len() == public_inputs.len() + 1: IC includes constant
    // - If IC.len() == public_inputs.len(): IC does NOT include constant

    let public_sum = if vk.ic.len() == public_inputs.len() + 1 {
        // IC includes constant at index 0
        // Compute: IC[0] + Σ public_i * IC[i+1]
        let mut sum = G1::from(vk.ic[0]);
        for (i, input) in public_inputs.iter().enumerate() {
            if i + 1 < vk.ic.len() {
                let ic_element = vk.ic[i + 1];
                let scalar = fq_to_fr(&input.value);
                let scaled = G1::from(ic_element) * scalar;
                sum += scaled;
            }
        }
        sum
    } else if vk.ic.len() == public_inputs.len() {
        // IC does NOT include constant
        // Compute: Σ public_i * IC[i]
        let mut sum = G1::zero();
        for (i, input) in public_inputs.iter().enumerate() {
            if i < vk.ic.len() {
                let ic_element = vk.ic[i];
                let scalar = fq_to_fr(&input.value);
                let scaled = G1::from(ic_element) * scalar;
                sum += scaled;
            }
    ...
    let public_gamma = Bn254::pairing(public_sum, vk.gamma_g2);
